...
class ProjectorIntegrationTest {
    ...
    fun `folds the log into latest state, history, and checkpoint transactionally`() {
        ...
        append(
            topic,
            Change("orders", listOf(Change.Entry("a", byteArrayOf(1)), Change.Entry("b", byteArrayOf(2)))),
            Change("orders", listOf(Change.Entry("a", byteArrayOf(3)))),
            Change("inventory", listOf(Change.Entry("x", byteArrayOf(9)))),
            Change("orders", listOf(Change.Entry("b", ByteArray(0), tombstone = true))),
        )
    ...
    fun `replaying already-applied records is a no-op`() {
        ...
        append(
            topic,
            Change("orders", listOf(Change.Entry("a", byteArrayOf(1)))),
            Change("orders", listOf(Change.Entry("a", byteArrayOf(2)), Change.Entry("b", byteArrayOf(4)))),
        )
        projector(topic).use {
            it.start()
            it.drain(2)
    ...
    fun `a second projector instance hits the checkpoint fence instead of interleaving`() {
        ...
        append(topic, Change("orders", listOf(Change.Entry("a", byteArrayOf(1)))))
